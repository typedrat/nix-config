...
in
{
  ...
  config = {
    resource = {
      ...
      authentik_provider_ldap = lib.mkMerge [
        ...
        (lib.mapAttrs'
          ...
          (
            lib.filterAttrs (
              _name: cfg: cfg.oauth2 != null && cfg.oauth2.backchannelLdap != null
            ) config.authentik.applications
          )
        )
      ];

      authentik_application = lib.mapAttrs (name: cfg: {
        inherit (cfg) name;
        slug = name;
        inherit (cfg) group;
        meta_icon = cfg.icon;
        meta_description = cfg.description;
        meta_launch_url = if cfg.oauth2 != null then cfg.oauth2.launchUrl else "";
        protocol_provider =
          if cfg.oauth2 != null then
            "\${ authentik_provider_oauth2.${name}-oauth2.id }"
          else if cfg.proxy != null then
            "\${ authentik_provider_proxy.${name}-proxy.id }"
          else
            "\${ authentik_provider_ldap.${name}-ldap.id }";
        backchannel_providers = lib.optional (
      ...
      authentik_policy_binding = lib.mkMerge [
        ...
        (lib.mapAttrs'
          ...
          (
            lib.filterAttrs (
              _name: cfg: cfg.ldap != null || (cfg.oauth2 != null && cfg.oauth2.backchannelLdap != null)
            ) config.authentik.applications
          )
        )
      ];
    ...
    authentik.outposts = {
      ldap.providers =
        lib.mapAttrsToList (name: _cfg: "\${authentik_provider_ldap.${name}-ldap.id}") (
          lib.filterAttrs (_name: cfg: cfg.ldap != null) config.authentik.applications
        )
        ++ lib.mapAttrsToList (name: _cfg: "\${authentik_provider_ldap.${name}-backchannel.id}") (
          lib.filterAttrs (
            _name: cfg: cfg.oauth2 != null && cfg.oauth2.backchannelLdap != null
          ) config.authentik.applications
        );

      proxy.providers = lib.mapAttrs (name: _cfg: "\${authentik_provider_proxy.${name}-proxy.id}") (
        lib.filterAttrs (_name: cfg: cfg.proxy != null) config.authentik.applications
      );
    };

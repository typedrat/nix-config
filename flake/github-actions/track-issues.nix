        ...
        {uses = "actions/checkout@v4";}
        {
          ...
          run = ''
            ...
            for ref in $refs; do
              ...
              # For NixOS/nixpkgs merged PRs, only consider resolved once in nixos-unstable
              if [ -n "$merged_at" ]; then
                owner_repo_lower=$(echo "$owner_repo" | tr '[:upper:]' '[:lower:]')
                if [ "$owner_repo_lower" = "nixos/nixpkgs" ]; then
                  merge_sha=$(gh api "repos/$owner_repo/pulls/$number" \
                    --jq '.merge_commit_sha' 2>/dev/null) || true

                  if [ -n "$merge_sha" ]; then
                    compare_status=$(gh api "repos/$owner_repo/compare/nixos-unstable...$merge_sha" \
            ...
            for ref in $refs; do
              ...
              if [ "''${ref_state[$ref]}" = "closed" ]; then
                ...
                open_rows+="| $link | $title_safe | ''${ref_status[$ref]} | ''${ref_locations[$ref]} |"
                open_rows+=$'\n'
              fi
            done
            ...
            body+=$'\n\n'"| Reference | Title | Status | Locations |"
            body+=$'\n'"|-----------|-------|--------|-----------|"

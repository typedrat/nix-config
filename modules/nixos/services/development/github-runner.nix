...
in {
  ...
  config = modules.mkMerge [
    (modules.mkIf cfg.enable {
      ...
      sops.secrets = lib.mkMerge (lib.mapAttrsToList (name: runnerCfg: {
      ...
      services.github-runners = lib.mapAttrs (name: runnerCfg: {
        enable = true;
        url = runnerCfg.url;
        tokenFile = config.sops.secrets."github_runner_tokens/${name}".path;
        name = name;
        extraLabels = runnerCfg.extraLabels;
      ...
      systemd.tmpfiles.rules = lib.mapAttrsToList (name: runnerCfg: 
        "d /var/lib/github-runners/${name} 0755 github-runner github-runner -"
      ) cfg.runners;
    })
  ];
}